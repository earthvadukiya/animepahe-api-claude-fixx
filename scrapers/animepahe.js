...
this.envCookies = process.env.COOKIES || "";
        this.cookiesRefreshInterval = 14 * 24 * 60 * 60 * 1000; // 14 days
        this.isRefreshingCookies = false;
        this.activeBrowser = null;
        this.cloudflareSessionCookies = null
        this.autoRefreshCookies = String(process.env.AUTO_REFRESH_COOKIES || 'false').toLowerCase() === 'true';
   ...
   async needsCookieRefresh() {
    if (this.envCookies && this.envCookies.includes("=")) {
        return false;
    }
    ...
    normalizeCookieString(cookieString = '') {
    ...
    async getCookies(userProvidedCookies = null, options = {}) {
        const allowRefresh = options.allowRefresh ?? this.autoRefreshCookies;
        ...
        if (userProvidedCookies) {
            if (typeof userProvidedCookies === 'string' && userProvidedCookies.trim()) {
                ...
                const normalizedUserCookies = this.normalizeCookieString(userProvidedCookies);
                Config.setCookies(normalizedUserCookies);
                return normalizedUserCookies;
            }
            throw new CustomError('Invalid user-provided cookies format', 400);
        }

        const normalizedEnvCookies = this.normalizeCookieString(this.envCookies);
        if (normalizedEnvCookies && normalizedEnvCookies.includes('=')) {
            console.log('Using cookies from environment variables');
            Config.setCookies(normalizedEnvCookies);
            return normalizedEnvCookies;
        }
        ...
        const ageInMs = Date.now() - (cookieData.timestamp || 0);
        if (allowRefresh && ageInMs > (this.cookiesRefreshInterval - 24 * 60 * 60 * 1000) && !this.isRefreshingCookies) {
            this.isRefreshingCookies = true;
            this.refreshCookies()
        ...
        const cookieHeader = this.normalizeCookieString(
            cookieData.cookies
    ...
    async fetchApiData(endpoint, params = {}, userProvidedCookies = null) {
        try {
            ...
            // Only retry with automatic cookies if explicitly enabled
            if (
                this.autoRefreshCookies &&
                !userProvidedCookies &&
    ...
    async scrapeAnimeInfo(animeId) {
        ...
        const cookieHeader = await this.getCookies(null, { allowRefresh: false });
    ...
    async scrapePlayPage(id, episodeId) {
        ...
        let cookieHeader = await this.getCookies(null, { allowRefresh: false });
        ...
        try {
            ...
            const hasEnvCookies = this.envCookies && this.envCookies.includes('=');
            ...
            if (
                ...
                await this.refreshCookies();
                cookieHeader = await this.getCookies(null, { allowRefresh: true });
