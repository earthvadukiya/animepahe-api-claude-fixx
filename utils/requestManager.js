...
class RequestManager {
    ...
    static async fetch(url, cookieHeader = '', type = 'default') {
        if (type === 'default') {
            return this.fetchHtml(url, cookieHeader);
        } else if (type === 'heavy') {
    ...
    static async fetchHtml(url, cookieHeader = '') {
        ...
        try {
            ...
            console.warn(`[fetchHtml] cloudscraper failed for ${url}:`, error.message);
        }
        ...
        if (response.status >= 400) {
            throw new CustomError(`Failed to fetch HTML (${response.status})`, response.status);
        }
